...
### 9단계 — 밸런스 시뮬레이션·최종 QA (진행 중 — 시뮬레이터 완성, 목표 시간은 부분 달성)
- [x] tools/sim 헤드리스 시뮬레이터 제작·검증(408→412 테스트, `test_sim_runner.gd` 4개 신규)
- [x] `bet_limit` 업그레이드의 실제 버그 발견·수정(비용 증가율이 효과 증가율보다 낮아 무제한 폭주 —
      다른 배율형 업그레이드와 반대였다)
- [ ] 목표 도달 시간 맞추기 — **B1→1F 는 맞췄지만(아래), 2F~엔딩은 구조적 한계로 이번 세션엔 못 맞춤**
- [ ] 클로버 약 230개 획득 확인 — 미달(평균 128개), 위와 같은 원인
