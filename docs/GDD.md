...
그동안 쌓아 온 경제로 통쾌하게 마무리 짓는 흔한 인크리멘탈 게임 패턴과도 맞다).

**9단계 후속 세션(2번째)**: 클로버 과다는 해결했다(원인은 자릿수 마일스톤 while 루프가 큰 당첨 한
번에 여러 단계를 건너뛸 때마다 반복 지급되던 것 — `CLOVER_PER_MILESTONE` 3→1, `STREAK_LENGTH`
5→20 으로 975개→227개, 목표 220~245개 달성). 1F→2F·PH→엔딩 구간은 구슬 재질 상한 추가 축소,
`bet_limit` 성장률 상향 두 가지를 더 시도했지만 개선하지 못했다(각각 거의 무변화, 진행 자체가
막힘 — 되돌림). 이 두 구간은 여전히 목표 미달로 남았다. 자세한 실측·시도 기록은
`docs/BALANCE_LOG.md` 참고. 최종 5회 평균(stable 전략): 1F 21.5분(목표 25~35, 근접) · 2F 22.8분
(목표 80~100, 미달) · 3F 132.6분(목표 150~180, 근접) · PH 177.5분(목표 225~255, 미달) · 엔딩 178.3분
(목표 270~330, 미달) · 클로버 227개(목표 220~245, **달성**).
