...
### 9단계 — 밸런스 시뮬레이션·최종 QA (진행 중 — 시뮬레이터 완성, 층 배율 재설계로 목표 시간 상당히 개선)
- [x] tools/sim 헤드리스 시뮬레이터 제작·검증(408→413 테스트, `test_sim_runner.gd` 4개 신규)
- [x] `bet_limit` 업그레이드의 실제 버그 발견·수정(비용 증가율이 효과 증가율보다 낮아 무제한 폭주 —
      다른 배율형 업그레이드와 반대였다)
- [x] 층 배율(payout_mult/bet_mult) 재설계 — 3F·PH 는 목표에 근접(146/165분, 192/240분), 1F→2F·
      PH→엔딩 두 구간은 구조적 한계로 여전히 미달(GDD 25-1장에 원인 상세)
- [ ] 클로버 약 230개 획득 확인 — 이번엔 초과(평균 975개, 스핀 수 자체가 훨씬 많아져서). 위 두 구간을
      더 손보면 같이 정상화될 가능성 높음 — 다음 세션 판단
- [x] bet_limit 로 인한 진행 불가(구슬 최저 한도 > 보유 칩) 버그를 실제 게임 코드에서 수정
      (`UpgradeService.purchase()` 뒤 `check_bankruptcy()`)
- [x] BIG WIN/JACKPOT 과다 발생 수정 — 배율 대신 "같은 번호에 구슬 2개 이상 몰아 걸어 적중" 기준으로 변경
- [x] 대출 계약서 팝업 얼룩 색상 버그(`WOOD`→문서화된 `MIST`) + 서명 모양 재설계(고리·플로리시)
- [ ] 실기기 최종 QA(텍스트 넘침 전수 재확인, 저사양·게임패드 확인) — 사용자 환경 필요, 미착수
...
### 8단계 진행 중 (2026-09-25) — 4/N: 전체 폴리시 감사

**한 일**: `docs/POLISH_CHECKLIST.md`(신규)에 전체 항목·상태를 정리했다(요약만 남기고 자세한 내용은
그 파일 참고). 새로 만든 것:
- `CursorTheme`(신규, `scenes/fx/cursor_theme.gd`) + `tools/art/gen_cursor.py`: 커스텀 마우스 커서 3종
  (기본/상호작용 가능/비활성, ART_BIBLE 16장). `SplashScreen._ready()` 에서 한 번 등록하면 이후 새로 추가되는
  모든 `BaseButton` 에 `disabled` 여부에 따라 자동으로 커서 모양이 붙는다(`AudioManager` 의 호버·클릭음
  자동 연결과 같은 `node_added` 신호를 별도로 구독 — 관심사가 달라 AudioManager 파일에는 안 섞었다).
- `project.godot` 커스텀 액션 5개에 게임패드 버튼 바인딩 추가(`spin`→A, `toggle_auto`→X, `open_skilltree`→Y,
  `switch_panel`→오른쪽 범퍼, `pause`→Start). Godot 기본 UI 액션은 원래 기본 조이패드 바인딩이 있었다.
- `SettingsManager`: 창이 최소화되거나 포커스를 잃으면(`NOTIFICATION_APPLICATION_FOCUS_OUT`/`_IN`)
  `Engine.max_fps` 를 10 으로 낮췄다가 복귀 시 되돌린다.
- 감사만 하고 새로 안 만든 것(이미 충분함을 확인): Godot 기본 테마 노출 없음, 버튼 4상태+사운드, 카운트업
  트윈, 패널 전환 시간, 픽셀 퍼펙트, 유휴 동작 반복 억제(`NeonText`/마담 벨벳 기존 패턴), 해상도 대응
  (viewport+keep+integer 스트레치).
- 테스트 신규: `tests/test_cursor_theme.gd`(4개), `test_settings_manager.gd` 에 최소화 프레임 제한 테스트 추가.
- 전체 **398 tests, 8975 checks, 0 failures**.
- `settings_game` 시나리오(새 튜토리얼 체크박스·다시 보기 버튼 포함) ko/en 재캡처로 텍스트 넘침 없음 확인.

**시도했지만 포기한 것(근거를 남겨 재시도 시 시행착오 방지)**
- **다크 테이블 무늬 레터박싱**: 검은 바 대신 펠트 무늬를 넣으려고 `rendering/environment/defaults/
  default_clear_color` 를 바꿔봤다. `tools/capture/capture.gd` 의 스크린샷은 내부 640×360 텍스처만 담아
  레터박스 바가 아예 안 보인다는 걸 먼저 발견했고(이 도구로는 검증 불가), `apt-get install x11-apps`
  (컨테이너에 없어서 새로 설치, netpbm 도 설치)로 `xwd` 를 써서 **실제 창을 통째로** 찍어봐야 했다.
  1600×1200(4:3) 창에서 확인한 결과 레터박스 바는 여전히 순수 검정(0,0,0)이었다 — 이 스트레치 모드
  (`viewport`)의 바깥 여백은 클리어 컬러 프로젝트 설정이 닿지 않는 다른 경로(디스플레이 서버 블릿 단계)로
  채워지는 것으로 보인다. 효과 없는 설정 변경은 되돌렸다. 진짜 구현하려면 `window/stretch/mode=
  "canvas_items"` 로 바꾸고 창 전체를 채우는 배경 + 수동 정수 배율·중앙 정렬 컨테이너를 직접 만드는 아키텍처
  변경이 필요한데, 1~7단계 전체가 지금의 "viewport" 스트레치를 전제로 좌표를 잡아 놔서 이 늦은 시점에는
  위험 대비 효과가 낮다고 판단해 범위에서 뺐다(GDD 21장에 기록).

**남은 이슈**
- 위 "포기한 것" 두 가지(레터박싱)와 저사양 성능·1시간 소크 테스트·실제 게임패드 확인은 이 클라우드
  컨테이너에서 할 수 없는 항목이라 `docs/POLISH_CHECKLIST.md` 의 "남은 일"에 남겨뒀다.
- ko/en 텍스트 넘침 전수 재확인은 이번 세션에서 바꾼 화면만 했다(7단계까지 이미 검수됨, 그 뒤 레이아웃
  변경 없음) — 9단계 최종 QA 때 전체를 다시 훑어볼 것.

**다음 작업(5/N — 스팀 출시 준비)이 알아야 할 것**
...
### 9단계 진행 중 (2026-09-25): 밸런스 시뮬레이터 제작 + 1차 튜닝(B1→1F 달성, 뒤는 구조적 한계 발견)

`tools/sim` 이 `.gitkeep` 뿐이던 상태에서 시작. 실제 게임 로직(GameState 등)을 그대로 돌리는 헤드리스
시뮬레이터를 새로 만들고, 그걸로 한 차례 밸런스를 조정했다. 전체 과정·수치 근거는 GDD 24장에 자세히
남겼다 — 여기는 요약과 다음 세션이 알아야 할 결정 사항.

**한 일**
- **`tools/sim/balance_sim.gd`(진입) + `tools/sim/sim_runner.gd`(실제 로직) 신규.** `godot --headless -s
  tools/sim/balance_sim.gd -- runs=5 upgrade_ratio=0.25 strategy=stable seed=<선택> max_hours=20` 로 실행.
  "성실히 플레이하는 유저"(안정 베팅, 감당 가능한 구슬 수만, 여유 있을 때만 층 이동, 남는 칩 25%로 가장
  싼 업그레이드 1개)를 B1부터 엔딩까지(또는 시간 초과까지) 재생하며 층별 도달 시간·클로버 획득량을 GDD
  7장 목표와 비교해 출력한다.
- 시뮬레이터를 만들며 직접 부딪힌 막힘 3가지를 코드로 고쳤다(전부 GDD 24장 표에 근거 포함):
  1. 구슬 수만큼 늘 다 걸면 베팅 한도가 자산보다 먼저 커져 스핀 자체가 막히는 경우 → 감당 가능한 구슬
     수만 건다.
  2. 층 비용만 딱 맞춰 올라가면 새 층의(즉시 확 뛰는) 최소 베팅도 못 내 스핀을 아예 시작 못 하고, 그러면
     `check_bankruptcy()`(스핀이 끝나야 도는 로직)로도 구제가 안 된다 → 층 비용 + 새 층 최소 베팅의
     1.5배가 모일 때만 올라간다.
  3. 스핀 1회의 큰 승리 직후 예산 전부를 그 자리에서 업그레이드에 재투자하면(재질→베팅 한도→재질→…)
     배율이 겹겹이 쌓여 게임 전체가 30여 스핀 만에 끝나 버림 → 스핀당 업그레이드 구매를 1회로 제한.
- **`bet_limit` 업그레이드의 실제 데이터 버그를 발견·수정**: `growth`(비용 증가율) 가 1.2 인데 효과
  (`max_bet_mult` MULT 1.35/레벨) 보다 낮았다 — 이 게임의 다른 모든 배율형(MULT) 업그레이드는 비용
  증가율이 효과보다 커서 스스로 한계가 있는데(예: 광택 1.7 vs 1.25) `bet_limit` 만 거꾸로였고, 상한도
  없어(`max_level=-1`) 무제한 폭주의 핵심 원인이었다(레벨 190대까지 순식간에 도달, 30여 스핀 만에 엔딩).
  `growth` 를 3.0 으로 올렸다(`data/upgrades/bet_limit.tres`, GDD 5장 표도 갱신).
- 위 수정들로 바뀐 값 때문에 깨진 테스트 6개를 고쳤다: `test_upgrade_service.gd`(경계값·MAX 구매량이
  growth 값에 하드코딩돼 있었음), `test_upgrade_panel.gd`(레벨 412/700 짜리 스냅샷이 growth=3.0 에서는
  비용이 `inf`/카드 폭 초과가 돼 60/110으로 낮춤).
- `tests/test_sim_runner.gd` 신규(4개) — 결과 모양, 짧은 시간 안전장치, 시드 재현성, 위 막힘 2번의
  회귀(3분 안에 15스핀보다 훨씬 많이 진행되는지)를 검사.
- 전체 **412 tests, 9060 checks, 0 failures**.

**핵심 발견(구조적 한계, 미해결) — 다음 세션이 결정할 것**
- 시뮬레이터로 5회 반복 측정한 결과: **B1→1F 는 목표(30분)에 근접**(실행마다 18~95분으로 변동은 크지만
  평균 35.5분)했지만, **1F 를 넘는 순간부터 2F·3F·PH·엔딩(목표 90/165/240/300분)은 실행마다 몇 분 안에
  전부 지나가 버린다**(5회 평균: 2F 36.7분, 3F 37.7분, PH 38.1분, 엔딩 38.2분 — 사실상 1F 직후 거의 바로).
  `upgrade_ratio` 를 0.15~0.5 사이로 바꿔 봐도 이 뒷부분 압축은 거의 그대로였다 — **플레이어 행동 가정의
  문제가 아니라 층마다 `payout_mult`/`bet_mult` 가 ×10~×10000 씩 뛰는 게 그때까지 쌓인 구슬 배율과
  곱해지면서 몇 스핀 만에 다음 층 비용을 넘겨 버리는 구조적 결과**다.
- 층 비용(1M/1T/1Sx/1No)과 엔딩 비용(1Dc)은 건드리지 않았다 — "숫자 표기 단위를 밟고 올라간다"는 서사
  장치라(GDD 7장·본문·크레딧·번역에 걸침) 밸런스만으로 깨기엔 GDD 를 다시 쓰는 수준의 결정이라 판단했다.
- 클로버 획득량도 같은 원인으로 미달(5회 평균 128개, 목표 약 230개) — 후반 층에서 보내는 스핀 수 자체가
  몇 스핀뿐이라 스트레치·마일스톤 클로버가 쌓일 시간이 없다.
- **다음 세션이 고를 두 선택지**(GDD 24장에도 기록): (a) "느린 초반 그라인드 + 빠른 후반 몰아치기"를
  그대로 게임의 맛으로 받아들이고 target_minutes 표 자체를 후반부만 현실적으로 낮춰 다시 쓴다, 또는
  (b) 7장의 층별 `payout_mult`/`bet_mult` 표를 다시 설계해(도약 폭을 줄이거나 완만하게) 뒤쪽도 그라인드가
  이어지게 한다. 이 시뮬레이터가 있어 어느 쪽을 고르든 같은 명령으로 바로 재검증할 수 있다.

**남은 이슈**
- 위 구조적 한계(2F 이후 시간 압축) — 사용자 결정 대기.
- `JackpotOverlay` 의 world-위-디밍 버그는 여전히 미해결(spawn_task 로 제안됨, task_02aff305).
- 실기기 최종 QA(텍스트 넘침 전수 재확인, 저사양·게임패드) — 사용자 환경 필요, 미착수.

**다음 작업이 알아야 할 것**
- `tools/sim` 은 완성돼 있고 재사용 가능 — 층 배율 표를 손보면 `godot --headless -s
  tools/sim/balance_sim.gd -- runs=10` 한 줄로 바로 재검증할 수 있다.
- `strategy=aggressive`(개별 숫자 올인)·`hot`·`martingale` 도 인자로 이미 지원한다 — "성실한 유저"
  기준선 말고 다른 플레이 스타일로도 비교해 볼 수 있다(이번 세션에선 `stable` 만 검증했다).

### 9단계 후속 (2026-09-25): 층 배율 재설계(b) + 사용자 버그 리포트 5건

사용자가 위 "다음 세션이 고를 선택지"에서 (b, 층별 배율 표 재설계)를 골랐고, 실제 플레이 중 발견한
버그 5건(대출 계약서 얼룩·서명, 진행 불가, BIG WIN/JACKPOT 과다)을 함께 요청했다. 전체 과정·실측 근거는
GDD 25장에 자세히 남겼다 — 여기는 요약.

**한 일**
- **층 배율(payout_mult/bet_mult) 재설계**(`data/floors/floor_{2,3,4}f}.tres` 등, GDD 25-1장): 1F 이후
  층마다 ×3(당첨)·×3(베팅)만 뛰도록 완만하게 바꿨다(원래는 ×100~×10000). 시뮬레이터로 5회 평균 재검증:
  1F 35.5분(목표 30)·2F 36.7분(목표 90, 미달)·**3F 146.6분(목표 165)·PH 191.5분(목표 240)** — 3F·PH 는
  원래(몇 분 안에 다 지나감)보다 훨씬 목표에 가까워졌다. 시행착오 중 "층의 배율은 그 층 도달 시간이
  아니라 다음 층까지 걸리는 시간을 좌우한다"는 인과관계를 처음엔 착각해 엉뚱한 층을 조정했다(GDD 25-1
  표에 기록) — 이후 마블 재질 비용 배율을 80배→150배로 올려보고, 1F 구슬 상한 점프(3→5)를 3→4로
  줄여보고, 재투자 비율을 낮춰봐도 1F→2F·PH→엔딩 두 구간만은 거의 안 바뀌는 걸 확인해, 이 두 구간은
  "이미 궤도에 오른 경제엔 문턱이 거의 없는" 구조적 결과로 결론 내렸다(층 이동 비용은 서사 장치라
  그대로 유지). 클로버는 전체 스핀 수가 크게 늘어(수십 회 → 수천 회) 목표(230개)를 오히려 초과했다
  (975개) — 위 두 구간이 정상화되면 자연히 맞을 가능성이 높아 별도로 손대지 않았다.
- **BIG WIN/JACKPOT 재설계**(`scripts/core/spin_outcome.gd`, `roulette_rules.gd`, GDD 25-2장): "너무
  자주 나온다"는 리포트대로, 배율 임계값(20배/100배) 대신 "같은 개별숫자에 구슬을 2개 이상 몰아 걸어
  다 같이 맞혔는지"로 바꿨다. `RouletteRules._max_same_number_concentration()`(신규)이 이긴 개별숫자 중
  같은 번호에 건 구슬 수의 최댓값을 계산 — 2개면 BIG, 3개 이상이면 JACKPOT. 더블 볼로 서로 다른 두
  번호가 우연히 같이 맞은 건 몰아걸기가 아니므로 제외했다. GOOD 은 그대로 배율(≥5배) 기준.
- **bet_limit 진행 불가 버그를 실제 게임에서도 수정**(`scripts/core/upgrade_service.gd`, GDD 25-3장):
  9단계 시뮬레이터 제작 중 발견한 문제(구매 직후 자산이 새 min_bet 밑으로 떨어지면 스핀을 아예 시작
  못 해 파산 구제도 못 받음)가 시뮬레이터만의 문제가 아니라 **실제 게임 코드에도 있었다**. `UpgradeService.
  purchase()` 끝에 `GameState.check_bankruptcy()` 를 추가해 스핀 뒤와 똑같이 즉시 구제되게 했다.
- **대출 계약서 팝업 수정**(`scenes/ui/contract_popup.gd`, GDD 25-4장): (1) 얼룩 점이 `Palette.WOOD`
  (고대비)로 찍혀 있었는데 ART_BIBLE 11-4 는 원래 `mist`(저대비)를 문서화하고 있었다 — 구현이 문서와
  다르던 버그. `MIST` 로 고치고 개수도 14→6개, 글줄 영역은 피하게 했다(캡처로 직접 확인). (2) 서명이
  사인파 하나짜리 지그재그라 심전도 그래프처럼 보였던 것을, 위/아래로 크게 휘는 획 + 이따금 낀 작은
  고리(필기체 e·l 처럼) + 마지막 밑줄 플로리시로 다시 짰다(`_signature_points()`/`_append_hump()`/
  `_append_loop()` 신규). `tools/capture` 의 `contract`/`stamp` 시나리오로 재캡처해 확인.
- 테스트: `test_resolve.gd`(등급 판정 재작성)·`test_spin_controller.gd`(BIG→GOOD 기대값 수정)·
  `test_upgrade_service.gd`(bet_limit 진행 불가 회귀 테스트 신규)·`test_contract_popup.gd`(기존 3개
  그대로 통과, 내부 구현만 바뀜) 갱신. 전체 **413 tests, 9065 checks, 0 failures**.

**남은 이슈**
- 1F→2F·PH→엔딩 두 구간의 구조적 시간 압축은 여전히 미해결(GDD 25-1장). 층 이동 비용 자체를 건드리지
  않는 한 이번에 시도한 방법(배율표·재질 비용·상한 점프·재투자 비율)으로는 못 고쳤다 — 사용자가 이
  제약을 받아들일지, 층 이동 비용까지 다시 설계할지 다음 세션에서 결정.
- `JackpotOverlay` 의 world-위-디밍 버그는 여전히 미해결(spawn_task 로 제안됨, task_02aff305).
- 실기기 최종 QA(텍스트 넘침 전수 재확인, 저사양·게임패드) — 사용자 환경 필요, 미착수.

**다음 작업이 알아야 할 것**
- BIG WIN/JACKPOT 이 이제 "몰아걸기" 전용이라, 색·홀짝 베팅만으로는 배율이 아무리 커도 GOOD 까지만
  간다 — 이후 새 배당 관련 기능을 추가할 때 "이것도 BIG/JACKPOT 급 연출이 필요한가"를 판단할 때 참고.
- `ContractPopup._signature_points()`/`_append_hump()`/`_append_loop()` 는 시드 기반 결정론적 생성이라
  `_quill_seed` 가 같으면 항상 같은 모양이 나온다 — 새 서명류 연출을 추가할 때 이 패턴을 재사용할 수 있다.

### 9단계 후속(2번째 세션, 2026-09-25) — 클로버 밸런스 마무리 + QA + 9단계 종료

**시작 전 확인**: `bash tools/setup_godot.sh` 로 Godot 4.3 설치 후 헤드리스 테스트 **413 tests, 9065
checks, 0 failures** 로 이전 세션(커밋 d52256e) 상태가 정상임을 확인.

**한 일**
- **클로버 과다 해결**(GDD 25-1장 후속, `docs/BALANCE_LOG.md` 신규 — 자세한 실측·시도 기록은 여기):
  원인은 `GameState._check_milestones()` 의 `while` 루프가 큰 당첨으로 칩이 한 스핀에 자릿수를 여러
  단계 건너뛸 때마다 반복 지급되던 것. `scripts/autoload/economy.gd` 의 `CLOVER_PER_MILESTONE` 3→1,
  `STREAK_LENGTH` 5→20 으로 조정(로직은 그대로, 상수만). 결과: 5회 평균 클로버 975.2개 → **227.0개**
  (목표 220~245개, 달성). 관련 테스트 2개 기대값 갱신(`test_game_state.gd::test_milestones_award_
  clovers`, `test_spin_controller.gd::test_win_streak_awards_clover_every_twenty`, 함수명도 5→20에
  맞게 변경).
- **1F→2F·PH→엔딩 미달 구간 추가 시도 2건, 모두 실패해 되돌림**(`docs/BALANCE_LOG.md` "시도 2"):
  (1) 1F 구슬 재질 상한 5→4 — 25-1장의 "상한 3→4" 시도와 같은 결론(거의 무변화) 재확인. (2)
  `bet_limit` 업그레이드 성장률 3.0→5.0 — 전체 경제가 너무 억눌려 3F 이후 진행이 20시간 안전장치
  안에서 아예 멈춤(클로버만 1752개로 폭증). 두 구간은 25-1장이 이미 내린 결론(층 이동 비용을 서사
  장치로 고정한 이상 구조적 한계) 그대로 남았다 — 데이터·상수 조정만으로는 더 개선하지 못했다.
- **최종 수치**: stable 전략 5회 평균 — 1F 21.5분(목표 25~35) · 2F 22.8분(목표 80~100, 미달) · 3F
  132.6분(목표 150~180) · PH 177.5분(목표 225~255, 미달) · 엔딩 178.3분(목표 270~330, 미달) · 클로버
  227.0개(목표 220~245, **달성**). 4개 전략(stable/aggressive/hot/martingale) 각 3회씩 추가 실측 —
  전략을 바꿔도 엔딩 도달이 169~179분대로 수렴(사용자 목표 "최적화 봇 엔딩≥200분" 미달, "느긋한 봇
  ≤450분"은 모든 전략에서 여유 있게 충족). `aggressive` 전략만 클로버 1104개로 여전히 과다(신규 발견,
  `docs/BUGS.md` 에 경미 이슈로 기록 — 공격적 베팅이 자릿수 도약을 더 자주 만들어 이번 튜닝 기준을
  벗어남).
- GDD 25-1장에 이번 세션 결과 요약 추가, `docs/BALANCE_LOG.md`(신규)에 전체 조정 근거·전후 수치·
  되돌린 시도까지 기록.
- `docs/BUGS.md`(신규): 이번 세션에 새로 발견한 경미 이슈(aggressive 클로버 과다, 두 구간 미해결)
  + 이전 단계들의 PROGRESS.md "남은 이슈"를 한곳에 모음 + 이번 세션에 시간 제약으로 못 한 QA 항목
  목록(전체 플레이스루 재캡처, 오프라인 30일/시계 역행/12구슬 더블볼 성능 등 — 치명 버그는 발견되지
  않았으나 직접 재현·캡처까지는 못 함).
- 전체 테스트 재확인: **413 tests, 9065 checks, 0 failures** (수정 전과 테스트 개수 동일, 2개 파일
  기대값만 조정).

**남은 이슈 (이번 세션 종료 시점)**
- 1F→2F·PH→엔딩 두 구간은 여전히 목표 미달(구조적, 위 참고) — 층 이동 비용 자체를 다시 설계하지
  않는 한 이 시뮬레이터·데이터 조정 방식으로는 더 못 좁혔다.
- `aggressive` 전략 클로버 과다(신규 발견, `docs/BUGS.md`) — 다음 세션에서 `STREAK_LENGTH` 를
  베팅 강도에 비례시키거나 클로버 지급 상한 로직을 추가하는 것을 검토할 만하다(이번 세션 범위 밖).
- 실기기 QA(5시간 실플레이, GPU/오디오 실장치 확인, xvfb 전체 플레이스루 재캡처)는 이번 세션도
  미착수 — `docs/BUGS.md` "확인 못한 항목" 참고, 사람이 직접 해야 한다.
- 아트 자산은 전부 절차적 생성(`tools/art/*.py`) 플레이스홀더 수준 그대로다 — 스팀 출시 전 실제
  아티스트 교체 여부는 사용자 판단이 필요하다(9단계 로드맵에 명시된 범위 밖).

### 9단계 체크리스트 (완료 기록)

- [x] `tools/sim/` 밸런스 시뮬레이터 제작(3봇×시드, CSV·리포트 대신 콘솔 리포트로 실측 — 별도
      아티팩트 파일은 만들지 않고 `docs/BALANCE_LOG.md` 에 결과를 옮겨 기록하는 방식을 택함)
- [x] bet_limit 업그레이드 폭주 버그 수정 (25-3장)
- [x] 층 배율 1차 재설계 (25-1장)
- [x] 대출 계약서 팝업 버그 수정 (25-4장)
- [x] BIG WIN/JACKPOT 과다 발생 수정 (25-2장)
- [x] 클로버 과다 수정 (이번 세션, 975→227개)
- [x] 밸런스 조정 근거·전후 수치 기록 (`docs/BALANCE_LOG.md`)
- [x] 최종 수치표 GDD 반영 (25-1장 갱신 — 값 자체가 바뀐 업그레이드/구슬/층/스킬 표는 이번 세션엔
      없음, 상수 2개만 변경돼 7장 표는 그대로 유효)
- [x] 전체 테스트 통과 확인(413/0 failures), 갱신 필요한 테스트 2개 보강
- [ ] `tools/capture` 자동 진행 모드로 처음부터 엔딩까지 전체 플레이스루 재캡처 — 미착수(시간 제약,
      `docs/BUGS.md` 참고)
- [x] 엣지 케이스 점검 — 기존 테스트(NaN/INF·저장 손상 복구 등)로 커버 확인, 신규 재현 실험은
      미착수(`docs/BUGS.md` 참고)
- [x] `docs/BUGS.md` 작성(경미 이슈 목록, 치명 버그 없음)
- [x] `docs/PROGRESS.md` 최종 정리(이 항목)
