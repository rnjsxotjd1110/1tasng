...
배율 = 순이익 ÷ 총 베팅액. 판정은 `SpinOutcome.tier`(GDD 4-2). BIG·JACKPOT 은 배율이 아니라 **같은
개별숫자에 몰아 건 구슬 수**로 정한다(9단계 변경, GDD 24장 "BIG/JACKPOT 재설계").
...
| GOOD | 5배 이상(몰아걸기가 아닌 색·홀짝 대박·개별숫자 1개 적중 포함) | NORMAL + 칩 파티클 20개, 1px 흔들림 0.15초 |
| BIG | 같은 개별숫자에 구슬 2개를 걸어 다 같이 적중 | "BIG WIN!" 네온 배너, 흰(ivory) 플래시(알파 0.25, 2프레임), 코인 분수 60개, 2px 흔들림 0.3초, 클로버가 상단 바로 비행 |
| JACKPOT | 같은 개별숫자에 구슬 3개 이상을 걸어 다 같이 적중 | 전체 화면: 배경 어둡게, 회전 금빛 광선, "JACKPOT" 글자 낙하, 코인 비, 2초 카운트업, 클릭으로 닫기 |
...
- 서명(9단계 재설계 — 단순 지그재그가 심전도 그래프처럼 보인다는 피드백으로 바꿈): 깃펜이 0.5초간
  서명란을 따라 그려짐(quill_sign 재생) — 폭을 3~4 조각으로 나눠 조각마다 위/아래로 크게 휘는 획과
  이따금 낀 작은 고리(필기체 e·l 처럼)를 잇고, 마지막엔 밑줄 플로리시로 마무리한다(`ContractPopup.
  _signature_points()`, 시드가 같으면 항상 같은 모양) → 빨간 발바닥 도장(`seizure_stamp.png` 와 별도, 서명용은 같은
...
## 18. 다크 테이블 레터박싱 (8단계 마무리, GDD 23장)

- 창 비율이 640×360(16:9)과 다르면(4:3 창의 위아래, 21:9 창의 좌우 등) 남는 여백을 검은 바 대신
  `LetterboxFit._Background` 가 그리는 어두운 테이블 무늬로 채운다.
- 무늬: `Palette.FELT_D` 바탕에 `Palette.WOOD_D` 16×16px 셀을 5% 확률로 흩뿌린다(고정 시드
  `20260925` — 창 크기가 바뀌어도 같은 느낌 유지). `felt_panel()`(3단계 베팅창 배경)과 같은 "펠트 +
  성긴 노이즈" 결을 새 텍스처 없이 절차적으로 재현했다.
- 게임 화면 자체(640×360 내용)는 항상 정수 배율로 중앙 정렬되고, 그 경계에 별도 테두리는 없다 —
  무늬 바탕색(`felt_d`, 매우 어두운 초록)이 게임의 어두운 톤과 자연스럽게 이어지도록 의도했다.
